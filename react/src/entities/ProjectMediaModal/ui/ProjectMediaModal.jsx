...
const ProjectMediaModal = ({ setOpasity, closeModal, project_id, isMobile = false }) => {
    ...
    const [createDocument] = useFetching(async (linkArg) => {
        return await KnowlegeBaseApi.createDocument({ origin: 'Файл', name: name, description: description, url: linkArg, grade: grade?.value?.number, semester: semester?.value, teacher: teacher?.value?.name, subject: subject?.value?.name, type: type.value, isPublic: isPublic, project_id: project_id })
    })
    const [sendFiles] = useFetching(async () => {
        return await KnowlegeBaseApi.sendFiles(files)
    })
    const [getProjectFiles] = useFetching(async () => {
        return await KnowlegeBaseApi.getProjectDocuments(project_id, 'Файл', 0, 25)
    })
    const handleSubmit = () => {
        ...
        <div className='modalWindow modalWindow_light'>
            ...
            <div className='modalWindow__content'>
                ...
                <div className="knowlegeBase-filters__content">
                    ...
                    />
                    {(!isMobile) &&
                        <div className={"field field_dark"}>
                            {files.length === 0 &&
                                <div className={classNames(
                                    "form__drop-zone",
                                    { ["on-drag"]: isDrag,
                                        ['form__drop-zone_dark']: true
                                    },
                                    []
                                )}
